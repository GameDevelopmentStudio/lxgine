...
LXTextureTGA::LXTextureTGA() {
}

LXTextureTGA::~LXTextureTGA() {
}

unsigned char *LXTextureTGA::newStringWithTGAFile(const char *filename, int &width, int &height) {
...
void LXTextureTGA::initWithPath(const char *path) {
    // check if file exists
    FILE* f = NULL;
    if (path == NULL) {
      printf("Couldn't load texture, file doesn't exist");
      return;
    }
    f = fopen(path, "rb");
    ...
    if (f){
        ...
        int width, height;
        unsigned char* data;
        data = newStringWithTGAFile(path, width, height);
